...
class DaqFitHandle:
    def __init__(self, dp, lp, resistivity, voltage, que:Queue) -> None:
        ...
        self.que_thread = queue.Queue() 
        self.file = None 
        self.filename = None 
        self.events_store = queue.Queue()
        self.thread = Thread(target = self.fitWorker)
        self.thread.start() 

    def setFile(self, filename):
        if self.file is not None:
            self.file.close()
        self.filename = filename[:-4] + "_rt.csv"
        self.file = open(self.filename, 'w')
        self.write("start(ms),end(ms),shape_o_rt,volume_o_rt,shape_p_rt,volume_p_rt\n")

    def fitWorker(self):
        while True:
            msg = self.que_thread.get() 
            if msg is None:
                break 
            elif isinstance(msg, tuple):
                self._proteinfit(*msg) 
                events_string = self.events_store.get_nowait() 
                if self.file is not None:
                    self.file.write(events_string)
    ...
    def append(self, data, eventstring: str, flag: bool):
        ...
        for i in events: 
            eventpoint = i.split(',')
            s = float(eventpoint[2])
            e = float(eventpoint[3])
            ...
            if end-start > 2:
                ...
                self.que_thread.put((data, I0, s, e, dI))
    ...
    def _proteinfit(self, data, I0, t1, t2, dI):
        buffer = io.BytesIO()
        self.events["dt"].append(t2-t1)
        ...
        self.events_store.put(f'{t1},{t2},{res["shape_o"]},{res["volume_o"]},{res["shape_p"]},{res["volume_p"]}\n')
        print(123)
