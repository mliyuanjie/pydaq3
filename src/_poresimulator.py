...
import random
from .cfunction import randomWalk, randomAngleWalk, randomWalkDt
import copy 
...
def calcumforrdc(n, dangle, angle_start, dipolefield):
    ...
    for i in range(1, n):
        pdirection = 1 / (1 + np.exp(dipolefield * 3.33356e-30 * (np.cos(data[i-1]-dangle) - np.cos(data[i-1]+dangle))/8.228666848e-21)) 
        direction = 1 
        #print(pdirection)
...
def Dt_PDF(t, d,l, v, dt):
    y = np.power(l / (4 * pi * d * np.power(t, 3)), 0.5) * np.exp(-1 * np.power(l - v * t, 2) / (4 * d * t))
    return y
...
def DI_PDFx(Imin, Imax, dipolewithe):
    ...
    c = (x>Imin) & (x<Imax) 
    y[c] = np.cosh(dipolewithe * 3.33356e-30 * np.sqrt((x[c] - Imin) / (Imax - Imin)) / 4.114333424e-21) / np.sqrt((x[c] - Imax) * (Imin - x[c])) 
    Area = np.trapz(y, x)
    if Area!=0:
        y = y / Area
    return (x[c], y[c])
...
def DI_PDFy(Imin, Imax, dipolewithe):
    ...
    y=np.zeros(x.shape, dtype = np.float64)
    c = (x>Imin) & (x<Imax) 
    y[c] = np.cosh(dipolewithe * 3.33356e-30 * np.sqrt((x[c] - Imax) / (Imin - Imax)) / 4.114333424e-21) / np.sqrt((x[c] - Imax) * (Imin - x[c])) 
    Area = np.trapz(y, x)
    if Area!=0:
        y = y / Area
    return (x[c], y[c])
...
class NanoporeSimulator:
    def __init__(self, poreradius = 10e-9, porelength = 30e-9, resistivity = 0.046, voltage = -0.1) -> None:
        #create a nanopore 
        ...
        self.baseline = voltage / (resistivity / (2 * poreradius) + (resistivity * porelength) / (pi * poreradius * poreradius)) 
        self.reactionrate = 0
    ...
    def addProtein(self, radiusx = 3e-9, radiusy = 3e-9, radiusz = 2e-9, charge =  -5, Diffusion_xy = 6e-11, Diffusion_r = 10000, Dipolement = 500):
        """
    ...
    def addReaction(self, bond_length = 3e-10, k_off = 3e-2, k_on = 0.1, reaction_radius = 7e-9):
        self.kon = k_on * np.exp(abs(self.efield * self.charge * bond_length) / 4.114333424e-21)
        self.koff = k_off * np.exp(abs(self.efield * self.charge * bond_length) / 4.114333424e-21)
    ...
    def simulateDt(self, maxtime = 100, dt = 1e-9):
        ...
        res = randomWalkDt(maxtime, dt, dx, probdirection, self.xmin, self.xmax, self.ymax, self.reactradus, self.kon, self.koff)
        return res
    ...
    def simulateIntraCurrent(self, angle_start = 0.0, n = 10000, dt = 2e-6):
        ...
        data = randomAngleWalk(n, dangle, angle_start, dipolefield)
        angle = data[-1]
        data = np.array(data)
        if self.shape <= 1:
            data = self.Imin + (self.Imax - self.Imin) * np.square(np.cos(data))
        else:
            data = self.Imax + (self.Imin - self.Imax) * np.square(np.cos(data))
        return (np.abs(data), angle)
    
    def simulateIntraCurrent3D(self, angle_start_x = 0.0, angle_start_y = 0.0, n = 10000, dt = 2e-6):
        dangle = sqrt(self.Dr * 2 * dt)
        dipolefield = self.efield * self.dipole
        data_x = randomAngleWalk(n, dangle, angle_start_x, dipolefield)
        data_y = randomAngleWalk(n, dangle, angle_start_y, dipolefield)
        angle_x = data_x[-1]
        angle_y = data_y[-1]
        data = np.cos(data_x) * np.cos(data_y)
        data = np.array(data)
        if self.shape <= 1:
    ...
    def simulateIntraEvent(self, maxtime = 0.001, dt = 1e-9):         
        ...
        for i in fragment:
            if resx is None:
                res = randomWalk(int(i[1] - i[0]), dx, dangle, self.dipolefield, probdirection, 0, 0, 0, self.xmin, self.xmax, self.ymax, self.borderwidth, 0.001, offrate, reacted)
                resx = np.array(res[0])
                resy = np.array(res[1])
                rescollision += res[4]
                ...
                y0 = resy[-1]
                angle0 = resangle[-1]
                res = randomWalk(int(i[1] - i[0]), dx, dangle, self.dipolefield, probdirection, x0, y0, angle0, self.xmin, self.xmax, self.ymax, self.borderwidth, 0.001, offrate, reacted)
            ...
            while s < 10000:              
                ...
                if s < 10000 and not state:
                    ...
                    if event_list[-1] >= 10000 - s:
                        tmp2 = self.simulateIntraCurrent(anglestart, 10000 - s, dt)
        ...
        for i in range(n):
            ...
            while s < 10000:              
                ...
                if s < 10000 and not state:
                    #print(event_list[-1])
                    if event_list[-1] >= 10000 - s:
                        tmp2 = self.simulateIntraCurrent3D(anglestartx, anglestarty, 10000 - s, dt)
                        data[s:10000] += tmp2[0] * scale_current
                        anglestartx = tmp2[1]
                        anglestarty = tmp2[2]
                        yield data
                        ...
                        if event_list[-1] == 0:
                            ...
                            continue
                        tmp = event_list.pop() 
                        tmp2 = self.simulateIntraCurrent3D(anglestartx, anglestarty, tmp, dt)
                        data[s:s+tmp] += tmp2[0] * scale_current
                        anglestartx = tmp2[1]
                        anglestarty = tmp2[2]
                        s += tmp
